...
import { articlesApi, templateApi, renderingTemplateApi, getImageUrl, type Product } from '../services/api';
import {
  PRINT_LAYOUTS,
  calculateLabelSize,
  mmToPx,
...
export default function LabelTemplateEditor() {
  ...
  const saveTemplate = async () => {
    try {
      ...
      if (error?.response?.data) {
        ...
        if (errorData.error === 'Invalid template') {
          errorMessage = `Template-Validierung fehlgeschlagen: ${errorData.message || 'Bitte überprüfen Sie alle Felder.'}`;
        } else if (errorData.error === 'Template already exists') {
          errorMessage = `Ein Template mit dieser ID existiert bereits. Verwenden Sie eine andere ID oder bearbeiten Sie das bestehende Template.`;
        } else if (errorData.error === 'Permission denied') {
          errorMessage = `Server-Berechtigung fehlt: ${errorData.message}. Bitte kontaktieren Sie den Administrator.`;
        } else if (errorData.error === 'No space left') {
          errorMessage = `Server-Speicher voll: ${errorData.message}. Bitte kontaktieren Sie den Administrator.`;
        }
  ...
  const handleConvertToRenderingTemplate = async () => {
    try {
      const templateName = prompt('Name für das Rendering Template:', `${template.name} (Rendering)`);
      if (!templateName) return;

      // Convert via API
      const response = await renderingTemplateApi.convert(template, templateName);

      if (response.success && response.data) {
        const convertedTemplate = (response.data as any).template;
        alert(`Template erfolgreich konvertiert!\n\nID: ${convertedTemplate.id}\nName: ${convertedTemplate.name}\n\nNavigiere zum Rendering Template Editor...`);

        // Navigate to rendering template editor
        navigate(`/rendering-template-editor?id=${convertedTemplate.id}`);
      }
    } catch (error: any) {
      console.error('Conversion error:', error);
      alert(`Fehler bei der Konvertierung: ${error.message || 'Unbekannter Fehler'}`);
    }
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      const convertResponse = await renderingTemplateApi.convert(template);

      if (!convertResponse.success || !convertResponse.data) {
        throw new Error('Konvertierung fehlgeschlagen');
      }

      const renderingTemplate = (convertResponse.data as any).template;

      // Now export as PDF
      const pdfResponse = await renderingTemplateApi.exportPdf(
        renderingTemplate.id,
        previewArticle
      );

      if (pdfResponse.success && pdfResponse.data) {
        const { pdf, fileName } = pdfResponse.data as any;
