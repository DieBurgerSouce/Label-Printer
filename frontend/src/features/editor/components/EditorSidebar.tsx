...
interface EditorSidebarProps {
  ...
  onUpdateTemplateSetting: <K extends keyof TemplateSettings>(key: K, value: TemplateSettings[K]) => void;
  ...
  return (
    <div className="w-64 bg-white border-r p-4 overflow-y-auto h-full scrollbar-thin">
      ...
      <div className="space-y-2">
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('text')}>
          <Type className="w-4 h-4 mr-2" />
          Überschrift
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('freeText')}>
          <Type className="w-4 h-4 mr-2" />
          Text
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('articleNumber')}>
          <Hash className="w-4 h-4 mr-2" />
          Artikelnummer
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('price')}>
          <DollarSign className="w-4 h-4 mr-2" />
          Preis
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('priceTable')}>
          <DollarSign className="w-4 h-4 mr-2" />
          Staffelpreise-Tabelle
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('description')}>
          <Type className="w-4 h-4 mr-2" />
          Beschreibung
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('image')}>
          <ImageIcon className="w-4 h-4 mr-2" />
          Produktbild
        </Button>
        <Button variant="outline" className="w-full justify-start" onClick={() => onAddElement('qrCode')}>
      ...
      <div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {template.settings.backgroundImage && (
              <div className="mt-2">
                <Label className="text-xs mb-1 block">Deckkraft: {Math.round(template.settings.backgroundImageOpacity * 100)}%</Label>
                <Input
                  type="range"
                  min="0"
                  max="1"
                  ...
                  onChange={e => onUpdateTemplateSetting('backgroundImageOpacity', parseFloat(e.target.value))}
                  className="w-full"
                />
              </div>
            )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e => onUpdateTemplateSetting('borderWidth', parseInt(e.target.value))}
                      className="h-8"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e => onUpdateTemplateSetting('borderRadius', parseInt(e.target.value))}
                      className="h-8"
                    />
                  </div>
                </div>
      ...
      <div>
        <TemplateRuleBuilder
          rule={template.rules}
          onChange={(newRule) => onUpdateTemplate({
            rules: newRule,
