...
export const parseTieredPrices = (article: any): { quantity: string; price: string }[] => {
  ...
  if (article?.tieredPricesText) {
    const lines = article.tieredPricesText.split('\n').filter((line: string) => {
...
export const getDisplayContent = (
  ...
  const isPlaceholder = (content: string): boolean => {
    return (
      !content || content.startsWith('{{') || content === 'Überschrift...' || content.trim() === ''
    );
  };
  ...
  switch (element.type) {
    case 'articleNumber':
      ...
      return previewArticle.articleNumber
        ? `Artikelnummer: ${previewArticle.articleNumber}`
        : element.content || '{{Artikelnummer}}';
