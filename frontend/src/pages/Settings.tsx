...
const SETTINGS_KEY = 'label_printer_settings';
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      try {
        ...
        setAppName(parsed.appName || 'Label Printer WebApp');
        setLanguage(parsed.language || 'en');
        setTheme(parsed.theme || 'light');
        setAutoSave(parsed.autoSave ?? true);
        setStoragePath(parsed.storagePath || '/data/labels');
        setCachePath(parsed.cachePath || '/data/cache');
        setMaxFileSize(parsed.maxFileSize || 50);
        setAutoCleanup(parsed.autoCleanup ?? true);
        setCleanupDays(parsed.cleanupDays || 30);
        setDefaultDpi(parsed.defaultDpi || 300);
        setDefaultFormat(parsed.defaultFormat || 'A4');
        setShowCutMarks(parsed.showCutMarks ?? true);
        setMaxLabelsPerPage(parsed.maxLabelsPerPage || 50);
        setEnableBatchProcessing(parsed.enableBatchProcessing ?? true);
      } catch (e) {
      ...
      <div className="space-y-6">
        {isLoading ? (
          <div className="card text-center py-12">
            ...
            <p className="text-gray-500 mt-4">Lade Einstellungen...</p>
          </div>
        ) : (
          <>
            {/* General Settings */}
            ...
            {activeTab === 'storage' && (
              <div className="space-y-6">
                <div className="card">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Verzeichnis für Labels und Bilder
                      </p>
                    </div>
                    ...
                    {autoCleanup && (
                      <div>
                        ...
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Lösche Cache-Dateien älter als X Tage
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="card bg-blue-50 border-2 border-blue-200">
                  <h3 className="font-semibold text-blue-900 mb-3">Speicher Statistik</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-blue-700">Anzahl Labels</p>
                      <p className="text-2xl font-bold text-blue-900">0</p>
                    </div>
                    <div>
                      <p className="text-sm text-blue-700">Belegter Speicher</p>
                      <p className="text-2xl font-bold text-blue-900">0 MB</p>
                    </div>
                    <div>
                      <p className="text-sm text-blue-700">Cache Größe</p>
                      <p className="text-2xl font-bold text-blue-900">0 MB</p>
                    </div>
                    <div>
                      <p className="text-sm text-blue-700">Verfügbar</p>
            ...
            {activeTab === 'print' && (
              <div className="space-y-6">
                <div className="card">
                  <h2 className="text-lg font-semibold text-gray-900 mb-4">Druck-Einstellungen</h2>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Standard DPI
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Maximale Anzahl Labels auf einer Seite
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label htmlFor="showCutMarks" className="text-sm text-gray-700">
                        Schnittmarken standardmäßig anzeigen
                      </label>
                    </div>
