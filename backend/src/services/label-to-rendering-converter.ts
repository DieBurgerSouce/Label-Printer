...
export class LabelToRenderingConverter {
  ...
  private static convertPriceElement(element: LabelElement, index: number): TemplateLayer {
    console.log(`   📋 Converting price element: id=${element.id}, content="${element.content}"`);
    const layer: TemplateLayer = {
      ...
      locked: false,
      order: index,
      position: { x: element.x, y: element.y, unit: 'px' as const },
      size: { width: element.width, height: element.height, unit: 'px' as const },
      rotation: 0,
      properties: {
        type: 'dynamic-text' as const,
        dataField: 'price',
        fieldType: 'price' as const,
  ...
  private static convertPriceTableElement(element: LabelElement, index: number): TemplateLayer {
    ...
    const layer: TemplateLayer = {
      ...
      properties: {
        ...
        fieldType: 'tieredPrice' as const,
